...
const TaxCalculator = () => {
  const [income, setIncome] = useState(0);  // Default value set to 0
  ...
  const calculateTax = (amount, slabs) => {
    ...
    // Updated logic for New Tax Regime to ensure no deduction till ₹1,200,000
    for (const slab of slabs) {
      if (amount <= 1200000 && slabs === newSlabs) {
        // No tax until ₹1,200,000 in New Tax Regime
        break;
      }
      const slabAmount = Math.min(Math.max(0, remainingAmount), slab.end - slab.start);
      totalTax += (slabAmount * slab.rate) / 100;
      remainingAmount -= slabAmount;
      if (remainingAmount <= 0) break;
    }
        ...
        <CardContent className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4 bg-white p-6 rounded-xl shadow-lg">
              ...
              <p className="text-lg">Deduction: {formatCurrency(oldTax)}</p>
            </div>
            ...
            <div className="space-y-4 bg-white p-6 rounded-xl shadow-lg">
              ...
              <p className="text-lg">Deduction: {formatCurrency(newTax)}</p>
            </div>
          </div>
          ...
          {showDetails && (
            <div className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4 bg-white p-6 rounded-xl shadow-lg">
                  <h3 className="font-semibold text-lg">Old Tax Regime Slabs</h3>
                  {oldSlabs.map((slab, index) => (
                    <div key={index} className="p-3 rounded-lg bg-blue-50">
                      <div className="flex justify-between text-sm">
                        <span>{formatCurrency(slab.start)} - {slab.end === Infinity ? 'Above' : formatCurrency(slab.end)}</span>
                        <span className="font-semibold">{slab.rate}%</span>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-4 bg-white p-6 rounded-xl shadow-lg">
                  <h3 className="font-semibold text-lg">New Tax Regime Slabs</h3>
                  {newSlabs.map((slab, index) => (
              ...
              <div className="h-80 bg-white p-6 rounded-xl shadow-lg">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={compareData}>
                    ...
                    <Legend />
                    <Bar dataKey="Old Regime" fill="#3b82f6" />
                    <Bar dataKey="New Regime" fill="#22c55e" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
